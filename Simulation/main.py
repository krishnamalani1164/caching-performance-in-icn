...
class Router(Node):
    ...
    def _init_(self, name, caching_policy='LRU'):
        super()._init_(name)
...
def setup_network():
    ...
    # Create a new network setup if loading fails or user chooses 'no'
    ...
    # Initialize content ID manager with publishers' data
    ...
    return routers, publishers, subscribers  # Correct return
...
def plot_merged_graph(policy_stats):
    ...
    # Create a figure with subplots to compare Cache Hit Ratio, Latency, and Hop Reduction
    ...
    plt.suptitle("Comparison of Caching Policies: LRU, LFU, FIFO, MRU")
    plt.tight_layout()
    plt.show()
